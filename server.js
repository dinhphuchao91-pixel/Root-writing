...
Phẩy which (chỉ cả ý mệnh đề trước — KHÔNG phải bổ nghĩa cho danh từ cụ thể):
...
app.post('/api/grade', async (req, res) => {
  ...
  if (!week || !essay || !essay.trim()) {
    return res.status(400).json({ error: 'Thiếu tuần học hoặc bài viết.' });
  }
  if (week < 1 || week > 9) {
    return res.status(400).json({ error: 'Tuần phải từ 1 đến 9.' });
  }
  ...
  try {
    ...
    try {
      ...
      return res.status(500).json({ error: 'AI không trả về JSON hợp lệ. Thử lại nhé.' });
    }
    ...
    if (err instanceof Anthropic.AuthenticationError) {
      return res.status(401).json({ error: 'API key không hợp lệ. Kiểm tra ANTHROPIC_API_KEY.' });
    }
    if (err instanceof Anthropic.RateLimitError) {
      return res.status(429).json({ error: 'Rate limit. Chờ một lúc rồi thử lại.' });
    }
    console.error(err);
...
app.listen(PORT, () => {
  console.log(`✅ Chấm bài ROOT đang chạy tại http://localhost:${PORT}`);
});
